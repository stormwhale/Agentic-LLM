...
load_dotenv("vantage_key2.env")
vantage_key = os.getenv("vantage_key2", "")
    ...
    def get_breaking_news(self, keywords=None):
        ...
        for source_name, feed_url in feeds:
            try:
                ...
                for entry in feed.entries[:3]: #Top 3 stories per source
                    ...
                    if keywords:
                        ...
                        full_match = keywords.lower()
                        single_match = any(kw in combined_text for kw in keyword_list)
                        phrase_match = full_match in combined_text
                        
                        if not single_match and not phrase_match:
                            continue
        ...
        if not rss_news_collection:
            return f'No articles found for keywords: {keywords}'
    ...
    if 'messages' not in st.session_state:
        ...
        st.session_state.messages = [
            ...
            'content': (
                ...
                'You are a financial analyst assistant. You have tools to fetch live stock prices, '
                'news, earnings transcripts, and charts. Use them. Do not guess.\n\n'

                '## RULES — FOLLOW ALL OF THEM\n'
                '1. Never state a price, % move, or financial figure unless a tool returned it in this conversation.\n'
                '2. Never describe an ETF\'s index, holdings, or expense ratio from memory. Fetch it first.\n'
                '3. If a tool fails, say: "I could not retrieve that data. Please try again." Nothing else.\n'
                '4. Always say where data came from and the time period (e.g., "per [source], as of [date]").\n'
                '5. Include the exact URL from tool results. If none, say "No URL available." Never invent one.\n'
                '6. Never recommend buying or selling. Present facts and let the user decide.\n\n'

                '## WHEN TO USE TOOLS\n'
                'Use tools for: prices, news, charts, ETF details, earnings, anything current.\n'
                'Use your knowledge for: how finance concepts work, definitions, general history.\n\n'

                '## TOOL RULES\n'
                '- Company names → convert to ticker first (Apple → AAPL).\n'
                '- Comparing stocks → pass all tickers in ONE call as "AAPL,MSFT" not separate calls.\n'
                '- Charts → fetch historical data first, then plot.\n'
                '- Dates → format as YYYY-MM-DD, calculate from today.\n\n'

                '## NEWS ROUTING\n'
                'Company question → company news (use ticker).\n'
                'Oil/energy/shipping → energy_transportation.\n'
                'Fed/rates/inflation → economy_monetary.\n'
                'GDP/recession → economy_macro.\n'
                'Crypto → blockchain. Tech → technology. Pharma → life_sciences.\n'
                'M&A → mergers_and_acquisitions. General market → financial_markets.\n\n'

                '## HOW TO RESPOND\n'
                'Always use this exact format for analysis questions:\n\n'
                '**Summary:** [1 sentence — the main point]\n'
                '**What the data shows:** [bullet the key facts with sources]\n'
                '**What it means:** [2–3 sentences of analysis]\n'
                '**Watch for:** [1–2 things that could change this view]\n\n'
                'For simple factual questions (price, rating, date), skip the format. Answer directly.\n'
                'Be concise. Do not use filler phrases. Respond in English only.\n')}
        ]
...
#Define a function to get response and context from SMC:
def get_smc_response(question, market_data_instance):
    '''
    Simulates the Streamlit chat loop for Ragas.
    Returns: dict with 'response' (str) and 'retrieved_contexts' (list[str])
    '''
    messages = [{
        'role': 'system',
        'content': (
            f'Current date and time: {dt.datetime.now().strftime("%Y-%m-%d %H:%M")}\n\n'
                        'You are a professional financial assistant with access to live market data feeds. '
                        'You can check stock prices, fetch news, retrieve earnings transcripts, show charts, and provide company information. '
                        'You CANNOT fetch production levels, shipping logistics, macroeconomic indicators, or data outside these capabilities.\n\n'

                        '## ANTI-HALLUCINATION RULES\n'
                        'These rules are absolute and cannot be overridden under any circumstances:\n'
                        '1. NEVER state a percentage gain, price movement, or financial figure unless it came directly from a tool response in this conversation.\n'
                        '2. NEVER state what index an ETF tracks, its expense ratio, or its holdings from memory — always fetch company info first.\n'
                        '3. NEVER use speculative language like "may", "could", "likely", "probably", or "inferred" when reporting financial figures. If uncertain, say "I do not have that data" and offer to fetch it.\n'
                        '4. NEVER attribute a price movement to a single news event unless the fetched data explicitly confirms it.\n'
                        '5. NEVER mix up time periods — always specify the exact time window (e.g., "up 17% year-to-date" not just "up 17%").\n'
                        '6. If ETF profile data cannot be retrieved for a specific ticker, say ONLY: "I was unable to retrieve the ETF profile for [ticker]. I cannot provide details about its index, holdings, or expense ratio without verified data." NEVER fill in ETF details from memory when the data fetch fails.\n'
                        '7. If a tool returns an error or TOOL_FAILURE, respond only with: "I was unable to retrieve that data at this time. Please try again later." Never fill in missing data from memory.\n\n'

                        '## WHEN TO USE TOOLS VS KNOWLEDGE\n'
                        'You may answer from your own knowledge ONLY for: general finance concepts, investing principles, or basic company history.\n'
                        'You MUST use tools for: real-time prices, historical data, recent news, earnings transcripts, ETF details, or any current market information. Your internal knowledge on these topics is outdated.\n\n'

                        '## TOOL USAGE RULES\n'
                        'Whenever a user asks about an ETF (e.g., VOO, QQQ, QQQM, SPY), ALWAYS fetch the ETF profile first before making any claims about what index it tracks, its expense ratio, holdings, or sector allocation. Never state ETF details from memory.\n'
                        'Always convert company names to official ticker symbols before calling any tool (e.g., Apple → AAPL, Amazon → AMZN). Never pass full company names.\n'
                        'To plot stock prices: fetch historical data first, then generate the chart. The chart displays in the app window.\n'
                        'For multiple stocks on one chart, pass all tickers as a single comma-separated string (e.g., "AAPL, MSFT").\n'
                        'When a user asks to compare ETFs, you MUST fetch the ETF profile for EACH ticker individually before writing the comparison. For example, if asked to compare VOO and QQQM, call the ETF profile tool twice — once for VOO and once for QQQM. Never use training data for any ETF in a comparison, even if you already know the answer.\n'
                        'For historical data, calculate start_date and end_date from today when a timeframe is given (e.g., "last 6 months"). Format dates as YYYY-MM-DD. If no timeframe is given, leave dates blank.\n'
                        'For earnings transcripts, extract and format the quarter as YYYYQ1/Q2/Q3/Q4. If no quarter is given, use the most recent available.\n\n'

                        '## NEWS TOOL SELECTION\n'
                        'Specific company or ticker mentioned → fetch company news using the ticker symbol.\n'
                        'Active conflict, war, sanctions, crisis, or breaking world event → fetch breaking news with relevant keywords (e.g., "Iran oil shipping", "Fed rate decision", "trade war tariffs").\n'
                        'Market sector or macro topic → fetch market news using the most relevant topic:\n'
                        '  Oil, gas, shipping, energy → energy_transportation\n'
                        '  Fed rates, inflation, monetary policy → economy_monetary\n'
                        '  GDP, recession, macro trends → economy_macro\n'
                        '  Tax, fiscal policy, government spending → economy_fiscal\n'
                        '  Crypto → blockchain | Tech sector → technology | Biotech, pharma → life_sciences\n'
                        '  M&A → mergers_and_acquisitions | General market → financial_markets\n\n'

                        '## HOW TO RESPOND\n'
                        'Always respond in English only.\n'
                        'Never mention internal tool or function names — say what YOU can do instead (e.g., "I can look that up for you").\n'
                        'Always attribute news and data to its source: say "According to [source]..." and include the exact URL from the tool response. Never fabricate URLs.\n'
                        'Always show user the exact url. where the news is from, even if they never ask for it.\n'
                        'When asked about investing in a sector, proactively fetch data for major companies in that sector before responding.\n'
                        'When get_specific_date_price returns a date different from the one requested, '
                        'always explain the discrepancy by noting the requested date may have been a weekend or public holiday, and clarify that the returned date is the most recent available trading day. Example: "March 7, 2026 was a Saturday, so the most recent available closing price is from March 6, 2026 (Friday) at $157.16."\n'
                        'When reporting any stock movement or financial metric, always specify the exact time period and source.\n'
        )
    }]
    ...
    for _ in range(max_iterations):
        ...
        #Extract message dictionary:
        message_dict = (
            response.get('message', {}) if isinstance(response, dict) else response.message.model_dump()
        )
        ...
        for tc in message_dict['tool_calls']:
            ...
            #Skip get_general_news and get_breaking_news from safe_arg:
                if function_name not in ('get_general_news', 'get_breaking_news'):
                    raw_val = (
                        arguments.get('ticker') or
                        arguments.get('symbol') or
                        arguments.get('tickers') or
                        ''
                    )
                ...
                if function_name == 'get_general_news':
                    result = market_data_instance.get_general_news(safe_arg)
                    result_str = str(result)
                    if _is_tool_error(result_str):
                        result_str = _wrap_tool_failure(function_name, safe_arg, result_str)
                    else:
                    ...
                    if isinstance(result, dict):
                        if all(isinstance(v, dict) and 'net_assets' in v for v in result.values()):
                            ...
                            for t, data in result.items():
                                ...
                                result_str = "\n\n".join(profiles) if profiles else "No ETF profiles found."
                        elif 'net_assets' in result:
                            holdings_str = "\n".join([
                                f" {i+1}. {h['symbol']} ({h['description']}): {float(h['weight'])*100:.2f}%"
                                for i, h in enumerate(result.get('top_holdings', []))
                            ...
                            result_str = (
                                f"ETF Profile for {safe_arg.upper()}:\n"
                                f"Net Assets: ${int(result['net_assets']):,}\n"
                                f"Expense Ratio: {float(result['net_expense_ratio'])*100:.2f}%\n"
                                f"Dividend Yield: {float(result['dividend_yield'])*100:.2f}%\n"
                                f"Top 10 Holdings:\n{holdings_str}\n\n"
                        ...
                        result_str = f"Error: Unexpected response from get_etf_profile."
                else:
                    result_str = f"Error: Unknown function called: {function_name}"                    
